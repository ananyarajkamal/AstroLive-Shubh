...
/**
 * Fetch with automatic retry and a 55-second timeout per attempt.
 * Handles Render cold starts (which can take up to 30s to wake up).
 */
...
export async function computeVahanRequest(
  inputData: VahanInputData
): Promise<Phase3ComputeResponse> {
  const payload = {
  ...
  try {
    response = await fetchWithRetry(`${API_BASE_URL}/vahan/compute`, {
      method: 'POST',
